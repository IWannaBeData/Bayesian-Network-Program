...
function setupEvents() {
  ...
  document.getElementById('highlightSelectedBtn').addEventListener('click', highlightSelectedElements);
  document.getElementById('highlightAllBtn').addEventListener('click', highlightAllElements);
...
function setTool(tool) {
  ...
  document.getElementById('highlightToolBtn').classList.toggle('active-tool', tool === 'highlight');
  document.getElementById('drawHint').classList.toggle('hidden', tool !== 'drawArrow');
  document.getElementById('highlightHint').classList.toggle('hidden', tool !== 'highlight');
  render();
}
...
function renderNodes() {
  ...
  state.nodes.forEach((node) => {
    ...
    el.addEventListener('mousedown', (event) => {
      ...
      if (event.shiftKey) toggleNodeHighlight(node.id);
    ...
    const ports = anchorCandidates(node);
    ports.forEach((p, idx) => {
      const dot = document.createElement('div');
      dot.className = 'connector-dot';
      dot.dataset.anchorIndex = String(idx);
      dot.style.left = `${p.x - node.x}px`;
      dot.style.top = `${p.y - node.y}px`;
      if (state.tool === 'drawArrow') dot.classList.add('active');
      if (state.dragNewEdge && state.dragNewEdge.targetId === node.id && state.dragNewEdge.endAnchorIndex === idx) {
        dot.classList.add('snap-target');
      }
      el.appendChild(dot);
    });
...
function renderEdges() {
  ...
  state.edges.forEach((edge) => {
    ...
    const edgeClasses = [];
    if (state.selectedEdgeIds.has(edge.id)) edgeClasses.push('edge-selected');
    if (state.highlightedEdgeIds.has(edge.id)) edgeClasses.push('edge-highlighted');
    if (edgeClasses.length) group.setAttribute('class', edgeClasses.join(' '));
  ...
  if (state.dragNewEdge) {
    ...
    if (source) {
      const start = anchorPoint(source, source, true, state.dragNewEdge.startAnchorIndex);
      let end = { x: state.dragNewEdge.currentX, y: state.dragNewEdge.currentY };

      if (state.dragNewEdge.targetId && Number.isFinite(state.dragNewEdge.endAnchorIndex)) {
        const targetNode = state.nodes.find((n) => n.id === state.dragNewEdge.targetId);
        if (targetNode) {
          const targetPort = anchorCandidates(targetNode)[state.dragNewEdge.endAnchorIndex];
          if (targetPort) {
            end = { x: targetPort.x, y: targetPort.y };
            const snap = document.createElementNS('http://www.w3.org/2000/svg', 'circle');
            snap.setAttribute('class', 'edge-snap-marker');
            snap.setAttribute('cx', targetPort.x);
            snap.setAttribute('cy', targetPort.y);
            snap.setAttribute('r', '6');
            edgeLayer.appendChild(snap);
          }
        }
      }
      ...
      preview.setAttribute('d', `M ${start.x} ${start.y} Q ${cx} ${cy} ${end.x} ${end.y}`);
...
function toggleEdgeHighlight(edgeId) {
  if (state.highlightedEdgeIds.has(edgeId)) state.highlightedEdgeIds.delete(edgeId);
  else state.highlightedEdgeIds.add(edgeId);
}

function highlightSelectedElements() {
  if (state.selectedNodeId) toggleNodeHighlight(state.selectedNodeId);
  if (state.selectedEdgeIds.size) {
    const allHighlighted = [...state.selectedEdgeIds].every((id) => state.highlightedEdgeIds.has(id));
    state.selectedEdgeIds.forEach((id) => {
      if (allHighlighted) state.highlightedEdgeIds.delete(id);
      else state.highlightedEdgeIds.add(id);
    });
  }
  render();
  updatePlainSummaryForCurrentSelection();
...
function highlightAllElements() {
  ...
  state.highlightedEdgeIds = new Set(state.edges.map((e) => e.id));
  render();
  updatePlainSummaryForCurrentSelection();
}
...
function startNewEdgeDraw(sourceNode, event) {
  const world = screenToWorld(event.clientX, event.clientY);
  state.pendingArrowSourceId = sourceNode.id;
  state.dragNewEdge = {
...
function deleteSelected() {
  if (state.selectedNodeId) {
    ...
    const removedEdgeIds = state.edges.filter((e) => e.parentId === id || e.childId === id).map((e) => e.id);
    state.edges = state.edges.filter((e) => e.parentId !== id && e.childId !== id);
    removedEdgeIds.forEach((edgeId) => state.highlightedEdgeIds.delete(edgeId));
  ...
  if (state.selectedEdgeIds.size) {
    ...
    removed.forEach((id) => state.highlightedEdgeIds.delete(id));
    state.selectedEdgeIds.clear();
